...
argument-hint: "<title> --type <name> [--priority high|medium|low] [--due YYYY-MM-DD]"
---
...
1. **Parse arguments from `$ARGUMENTS`:**
   ...
   - `--type <name>` — required; the type of work label (e.g. work, personal, finances, learning)
...
3. **Find next id:**
   - Glob `tasks/**/*.md` recursively across all type subdirectories
...
5. **Write `tasks/<type>/TASK-NNN-<slug>.md`** (create the type subfolder if it doesn't exist):
   ```yaml
   ---
   id: TASK-NNN
   title: <title>
...
6. **Log** to `wiki/log.md`:
   ...
   - Created: tasks/<type>/TASK-NNN-<slug>.md
   ```
...
8. **Confirm** to the user: "Created **TASK-NNN** — <title> (`tasks/<type>/TASK-NNN-<slug>.md`)"
